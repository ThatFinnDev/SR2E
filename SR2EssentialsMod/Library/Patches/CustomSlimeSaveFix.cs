...
public class AppearancesSaveFix
{
    public static bool Prefix(AppearancesModel __instance, ref AppearancesV01 __result, SaveReferenceTranslation saveRef)
    {
        ...
        {
            gameContext.AutoSaveDirector._saveReferenceTranslation.RefreshIfNotFound(selection.Key);
...
[HarmonyPatch(typeof(GameV06), nameof(GameV06.LoadSummaryData))]
public static class EnsureSlimesSavableFix
{
    static void Prefix()
    {
        foreach (var actor in savedIdents)
        {
            gameContext.AutoSaveDirector._saveReferenceTranslation.RefreshIfNotFound(actor.Value);
        }
    }
}